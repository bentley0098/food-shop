...
- `seed.sql` with a handful of **generically named** ingredients ("Chicken breast", not "Tesco Finest…" — the seed is the naming example everyone copies) and 3–4 realistic recipes so the planner and list have data to chew on from Phase 2 onward. **Seed data is a deliverable, not a convenience** — Phase 3's aggregation is untestable by hand without it.
